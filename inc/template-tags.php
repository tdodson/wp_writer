...
if ( ! function_exists( 'underscores_sass_posted_on' ) ) :
	...
	function underscores_sass_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'underscores_sass' ),
	...
	function underscores_sass_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'underscores_sass' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
